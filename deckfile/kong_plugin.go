...
func NewKongPlugin(data map[string]interface{}, deckfile *DeckFile) (*KongPlugin, error) {
	...
	id, _ := filebasics.GetStringField(data, "id")
	serviceRef, _ := filebasics.GetStringField(data, "service")
	routeRef, _ := filebasics.GetStringField(data, "route")
	consumerRef, _ := filebasics.GetStringField(data, "consumer")
