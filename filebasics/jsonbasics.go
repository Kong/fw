...
// occurences determines the maximum number of items to remove, use -1 for unlimited.
...
func RemoveObjectFromArrayByFieldValue(inArr interface{},
	fieldName string, fieldValue string, occurences int) (interface{}, int, error) {

	if occurences < -1 {
		panic(fmt.Sprintf("occurences cannot be smaller than -1, got %d", occurences))
	}

	if inArr == nil || occurences == 0 {
		return inArr, 0, nil
	}
	...
	for _, entry := range arr {
		...
		if err == nil {
			...
			if err == nil && strValue == fieldValue && occurences > 0 {
				isMatch = true
			}
		}
